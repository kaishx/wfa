...
# keeping this for myself as i have DLL trouble with cpp, but for those using this, you may comment this block out, im honestly not too sure if this bug will reoccur on all machines that run this code.
if os.name == 'nt':
    try:
...
def bt_strat(data, z_entry, z_exit, z_stop_loss, cptl, txfee, slippage, asset_a, asset_b, abs_stop):
    ...
    cpp_array = np.array(cpp_pnl_array)

    if len(cpp_array) != len(temp_data):
        raise ValueError(
            f"!!! error: C++ output length ({len(cpp_array)}) mismatches Python data length ({len(temp_data)}) !!!")

    temp_data['Pnl'] = cpp_array
    ...
    trade_count = (cpp_array != 0).sum()
    trade_log = []
    all_pnls = pd.DataFrame({'Time': temp_data.index, 'PnL': cpp_array})
...
def run_opt(data, cptl, txfee, slippage, asset_a, asset_b, abs_stop, hurstMax):
    close_a = data[f'Close_{asset_a}'].values.astype(np.float64)
    close_b = data[f'Close_{asset_b}'].values.astype(np.float64)
    z_score = data['Z_Score'].values.astype(np.float64)
    beta    = data['Beta'].values.astype(np.float64)
    hurst   = data['Hurst'].values.astype(np.float64)
    vol = data['Spread_Std'].values.astype(np.float64)

    grid_entry = np.array(Z_ENTRY_GRID, dtype=np.float64)
    grid_exit  = np.array(Z_EXIT_GRID, dtype=np.float64)
    grid_sl    = np.array(Z_STOP_LOSS_GRID, dtype=np.float64)

    result = cpp_accelerator.run_optimization_core(
        close_a, close_b, z_score, beta, hurst, vol,
        grid_entry, grid_exit, grid_sl,
        cptl, txfee, slippage, abs_stop, hurstMax,
        float(BARS_PER_YEAR)
    )

    best_sharpe = result[3]

    if best_sharpe <= -50.0:
        return None, 0.0, 0.0

    best_params = (result[0], result[1], result[2])
...
def rolling_bt(processed_full_data, rolling_window_bars, optimization_window_bars, shift_days, cptl, txfee, slippage, asset_a, asset_b):
    ...
    while i < len(processed_full_data):
        ...
        print(f"ADF Test Result: p-value={adf_p_value:.4f} (Required < {ADF_P_VALUE_THRESHOLD:.2f})")
        ...
        try:
            if all_oos_pnls:
                ...
                print(f"Persistence: Saved {len(current_oos_df)} PnL points.")
        except Exception as e:
    ...
    YEARS_TESTED = max(TRADING_DAYS_IN_PERIOD / TRADING_DAYS_PER_YEAR, 1 / TRADING_DAYS_PER_YEAR)
    ...
    if len(trade_pnls) == 0:
        ...
        trades_per_year = len(trade_pnls) / YEARS_TESTED
        if std_trade != 0:
    ...
    return {
        'Total_PnL': final_pnls_df['Cumulative_Pnl'].iloc[-1],
...
if __name__ == "__main__":
    try:
        print("C++ Module (cpp_accelerator) loaded successfully.")
